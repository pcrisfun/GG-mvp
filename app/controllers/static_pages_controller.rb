class StaticPagesController < ApplicationController
  def home
    @featured_events = Event.where(featured: true).where("begins_at >= :recent").where("begins_at >= :recent", {recent: Date.today-30.days}).sort_by { |e| e.begins_at }.reverse!.take(4)
