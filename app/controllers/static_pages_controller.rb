class StaticPagesController < ApplicationController
  def home
    ...
    if @featured_events.count < 4
      @events1 = Event.where(datetime_tba: false, featured: false, state: ['accepted']).sort_by { |e| e.begins_at }.reverse!.take(4-@featured_events.count)
      @events2 = Event.where(datetime_tba: false, featured: nil, state: ['accepted']).sort_by { |e| e.begins_at }.reverse!.take(4-(@featured_events.count + @events1.count))
